...
func Setup(ctx context.Context, r *mux.Router, cfg *config.Config, c Clients) {
	...
	if cfg.EnableMultivariate {
		r.StrictSlash(true).Path("/filters/{filterID}/dimensions/change").Methods("GET").HandlerFunc(handlers.ChangeDimensions(c.Render, c.Filter, c.Dataset, c.Population))
	}
	...
	r.StrictSlash(true).Path("/filters/{filterID}/dimensions/geography/coverage").Methods("GET").HandlerFunc(handlers.GetCoverage(c.Render, c.Filter, c.Population, c.Dataset))
