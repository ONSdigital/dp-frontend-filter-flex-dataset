...
.PHONY: build
build: generate-prod
	go build -tags 'production' $(LDFLAGS) -o $(BINPATH)/dp-frontend-filter-flex-dataset

.PHONY: debug
debug: generate-debug
	go build -tags 'debug' $(LDFLAGS) -o $(BINPATH)/dp-frontend-filter-flex-dataset
	HUMAN_LOG=1 DEBUG=1 $(BINPATH)/dp-frontend-filter-flex-dataset
...
ifeq ($(LOCAL_RENDERER_IN_USE), 1)
	$(eval CORE_ASSETS_PATH = $(shell cat go.mod | grep -v "replace" | grep -w "github.com/ONSdigital/dis-design-system-go/v2" | awk '{print $2}' | tr -d '"'))
else
	$(eval APP_RENDERER_VERSION=$(shell cat go.mod | grep -v "replace" | grep "github.com/ONSdigital/dis-design-system-go/v2"  | cut -d ' ' -f2 ))
